...
namespace LoginService.Database
{
    public class LoginDatabaseHandler : ILoginDatabaseHandler, IDisposable
    {
        ...
        {
            _user = new User();
            using (SqliteConnection _connection = new SqliteConnection(DataSourceString))
            {
                _connection.Open();
                using (SqliteCommand _cmd = new SqliteCommand(
                           "CREATE TABLE IF NOT EXISTS Login (id INTEGER NOT NULL UNIQUE, username	TEXT NOT NULL, passwordhash	TEXT NOT NULL, email TEXT NOT NULL, license	TEXT NOT NULL, PRIMARY KEY(id AUTOINCREMENT));",
                           _connection))
                {
                    _cmd.Prepare();
        ...
        public bool UsernameExists(MessageDto messageDto)
        {
            using (SqliteConnection _connection = new SqliteConnection(DataSourceString))
            {
                ...
                using (SqliteCommand _cmd =
                       new SqliteCommand("SELECT username FROM login WHERE username=@username", _connection))
                {
